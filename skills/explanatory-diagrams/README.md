...
使う場面：いくつかの段階にまたがる処理で、コピーして持ち回っていた値を削除し、どの処理も元の値を読むようにするリファクタリング。データ・読む処理・結果を帯に分け、変更前と変更後を 1 枚に重ねる。削除したものは橙の点線と打ち消し線、新しく読む値とそれによって変わる値は青で示し、変わる値は具体例で見せる。
